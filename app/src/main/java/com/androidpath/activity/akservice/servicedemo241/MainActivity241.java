...
//		startThread();
    ...
    private void startThread() {
        Thread thread = new Thread() {
            ...
            public void run() {
                super.run();
                System.out.println("xxxxxxxxxxxxx MainActivity241 thread run");
                while (isRunning) {
                    if (System.currentTimeMillis() - last > 1000) {
                        last = System.currentTimeMillis();
                        System.out.println("xxxxxxxxxxxx MainActivity241 Thread in " + System.currentTimeMillis() / 1000);
                    }
                }
            }
        };
        thread.setName("MainActivity241 Thread");
        thread.start();
    }
