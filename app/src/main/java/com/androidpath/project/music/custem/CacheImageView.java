package com.androidpath.project.music.custem;
...
import com.androidpath.project.music.utils.ImageUtil;
...
public class CacheImageView extends ImageView implements DownUtils.OnDownComplete {

	/**
	 * 一级缓存--磁盘缓存 参数：表示这个内存缓存最大的空间，通常我们给运行时内存1/8
	 */
	private static LruCache<String, Bitmap> lruCache = new LruCache<String, Bitmap>(
			(int) (Runtime.getRuntime().maxMemory() / 8)) {

		/**
		 * lruCache内存满了以后，再往里面添加数据时， lruCache会按照最近时间最少使用的策略，进行数据移除，并且回调该方法
		 */
		@Override
	...
	/**
	 * java的四种引用 强引用：现在所用的所有引用都是强引用，如果程序运行时，内存不足的话，系统宁可抛出内存溢出的异常，也不会去回收强引用的对象
	 * 软引用：如果程序运行时，内存不足的话，会回收软引用对象 弱引用：无论内存紧不紧张，只要被GC发现，立刻回收
